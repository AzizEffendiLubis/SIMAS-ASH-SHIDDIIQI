...
<div class="page-header" style="display:flex;align-items:center;justify-content:space-between;">
    ...
    <div style="display:flex;gap:10px;">
        <a href="{{ route('repairs.edit', $repair) }}" class="btn btn-primary"><i class="fas fa-pen"></i> Update</a>
...
<div style="display:grid;grid-template-columns:2fr 1fr;gap:20px;align-items:start;">
    <div style="display:flex;flex-direction:column;gap:20px;">
        ...
        <div class="card">
            <div class="card-body" style="display:flex;align-items:center;gap:16px;padding:20px;">
                <div style="width:56px;height:56px;border-radius:14px;background:{{ $repair->status === 'Selesai' ? '#dcfce7' : ($repair->status === 'Sedang Diperbaiki' ? '#e0f2fe' : '#fef9c3') }};display:flex;align-items:center;justify-content:center;font-size:22px;color:{{ $repair->status === 'Selesai' ? '#16a34a' : ($repair->status === 'Sedang Diperbaiki' ? '#0369a1' : '#a16207') }};">
                    <i class="fas fa-{{ $repair->status === 'Selesai' ? 'circle-check' : ($repair->status === 'Sedang Diperbaiki' ? 'gear fa-spin' : 'clock') }}"></i>
                </div>
                <div>
                    <p style="font-size:13px;color:#64748b;margin-bottom:2px;">Status Perbaikan</p>
                    <span class="badge badge-{{ $repair->status === 'Selesai' ? 'success' : ($repair->status === 'Sedang Diperbaiki' ? 'info' : 'warning') }}" style="font-size:14px;padding:5px 14px;">{{ $repair->status }}</span>
                </div>
                @if($repair->tanggal_selesai)
                <div style="margin-left:auto;text-align:right;">
                    <p style="font-size:12px;color:#94a3b8;">Selesai pada</p>
                    <p style="font-weight:600;font-size:14px;">{{ $repair->tanggal_selesai->format('d M Y') }}</p>
                </div>
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div>
                        ...
                        <p style="font-weight:600;font-size:15px;">{{ $repair->asset->nama_barang ?? '-' }}</p>
                        <p style="font-size:12px;color:#94a3b8;">{{ $repair->asset->kode_barang ?? '' }}</p>
                    </div>
                    <div>
                        <p style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Lokasi Barang</p>
                        <p style="font-weight:500;">{{ $repair->asset->lokasi_barang ?? '-' }}</p>
                        <p style="font-size:12px;color:#94a3b8;">{{ $repair->asset->unit_kerja ?? '' }}</p>
                    </div>
                    <div>
                        <p style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Dilaporkan Oleh</p>
                        <p style="font-weight:500;">{{ $repair->pelapor->name ?? '-' }}</p>
                    </div>
                    <div>
                        <p style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Tanggal Laporan</p>
                        <p style="font-weight:500;">{{ $repair->tanggal_laporan->format('d M Y') }}</p>
                    </div>
                    <div>
                        <p style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Ditangani Oleh</p>
                        <p style="font-weight:500;">{{ $repair->teknisi->name ?? '<span style="color:#94a3b8;font-style:italic;">Belum ditugaskan</span>' }}</p>
                    </div>
                    @if($repair->biaya_perbaikan)
                    <div>
                        <p style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Biaya Perbaikan</p>
                        <p style="font-weight:700;font-size:16px;color:#2563eb;">Rp {{ number_format($repair->biaya_perbaikan, 0, ',', '.') }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div style="margin-top:16px;padding-top:16px;border-top:1px solid #f1f5f9;">
                    ...
                    <p style="font-size:13.5px;color:#374151;background:#fef9c3;border-radius:8px;padding:12px 14px;border:1px solid #fde68a;line-height:1.6;">{{ $repair->deskripsi_kerusakan }}</p>
                </div>
                ...
                <div style="margin-top:14px;">
                    ...
                    <p style="font-size:13.5px;color:#374151;background:#f0fdf4;border-radius:8px;padding:12px 14px;border:1px solid #bbf7d0;line-height:1.6;">{{ $repair->tindakan_perbaikan }}</p>
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-body" style="text-align:center;">
            ...
            <img src="{{ Storage::url($repair->foto_kerusakan) }}" alt="Foto Kerusakan"
            ...
            @if($repair->asset && $repair->asset->foto)
            <div style="margin-top:14px;padding-top:14px;border-top:1px solid #f1f5f9;">
                ...
                <img src="{{ Storage::url($repair->asset->foto) }}" alt="Foto Aset"
                     style="width:100%;border-radius:10px;border:1px solid #e2e8f0;object-fit:cover;">
            </div>
            @endif
        </div>
