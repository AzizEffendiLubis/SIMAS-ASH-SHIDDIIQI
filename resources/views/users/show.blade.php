...
@section('page-parent', 'Detail Pengguna')
...
<div class="page-header-row">
    <div class="ph-left">
        ...
        <p>Username: <strong style="color:var(--gray-700);">{{ $user->username }}</strong></p>
    </div>
    <div class="ph-right">
...
{{-- Catatan: pakai class "dash-two-col" (sudah punya rule responsive di layout:
     2 kolom desktop, 1 kolom mobile). Jangan inline grid-template-columns lagi
     karena inline style selalu menang melawan @media dan menghalangi stack di HP. --}}
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <span class="badge badge-{{ $user->status }}">{{ $user->status_label }}</span>
            </div>
            <div class="card-body">
                @php
                ...
                <div class="user-identity-row">
                    <div class="role-avatar" style="background:{{ $avatarColor }};">
                        {{ strtoupper(substr($user->name, 0, 1)) }}
                    </div>
                    <div>
                        <p class="identity-name">{{ $user->name }}</p>
                        <p class="identity-jabatan">{{ $user->jabatan ?? '-' }}</p>
                    </div>
                </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                @else
                    {{--
                        $allMenus di-pass dari controller show().
                    ...
                    <div class="menu-access-grid">
                        @foreach($allMenus as $key => $label)
                            ...
                            <div class="menu-access-item {{ $hasAccess ? 'granted' : 'denied' }}">
                                <i class="fas fa-{{ $hasAccess ? 'circle-check' : 'circle-xmark' }}"></i>
                                {{ $label }}
                            </div>
                        @endforeach
                    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="card">
            ...
            <div class="card-body role-card-body">
                <div class="role-icon" style="background:{{ $avatarColor }};">
                ...
                <div class="role-main">
                    ...
                    <p class="role-name">{{ $user->role_label }}</p>
                    <p class="role-desc">
                        @switch($user->role)
