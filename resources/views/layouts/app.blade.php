...
<head>
    ...
    <title>SIMAS – @yield('title', 'Sistem Informasi Manajemen Aset')</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-brand">
        <div class="brand-logo">
            <img src="{{ asset('images/10505884-5.jpg') }}" alt="Logo SIMAS">
        </div>
    ...
    <nav class="sidebar-nav" role="navigation" aria-label="Menu utama">
        ...
        @if(auth()->user()->canAccess('daftar_aset'))
        ...
        @if(auth()->user()->canAccess('manajemen_pengguna') || auth()->user()->canAccess('log_aktivitas') || auth()->user()->canAccess('master_data'))
        <div class="nav-section-label" style="margin-top:4px;">Administrasi</div>
        @endif

        @if(auth()->user()->canAccess('manajemen_pengguna'))
        ...
        @if(auth()->user()->canAccess('master_data'))
        <a href="{{ route('masterdata.index') }}"
           class="nav-link {{ request()->routeIs('masterdata.*') ? 'active' : '' }}">
    ...
    <div class="sidebar-footer">
        <div class="sidebar-user">
            ...
            <div class="su-info">
                <div class="su-name">{{ Str::limit(auth()->user()->name, 22) }}</div>
                <div class="su-role">{{ auth()->user()->role_label }}</div>
            </div>
        </div>
    ...
    <header class="topbar" role="banner">
        <div class="topbar-left">
            ...
            <div class="topbar-breadcrumb">
                ...
                @hasSection('page-parent')
                    <span class="separator"><i class="fas fa-chevron-right" style="font-size:10px;"></i></span>
                    <span class="parent-title">@yield('page-parent')</span>
                @endif
            </div>
        </div>

        <div class="topbar-right">
            <div class="topbar-user" title="{{ auth()->user()->name }} — {{ auth()->user()->role_label }}">
                <div class="tu-avatar">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</div>
                <div class="tu-info">
                    <div class="tu-name">{{ Str::limit(auth()->user()->name, 22) }}</div>
                    <div class="tu-role">{{ auth()->user()->role_label }}</div>
                </div>
            </div>
        </div>
    </header>
